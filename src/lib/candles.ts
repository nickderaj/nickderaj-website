...
export function generateCandles(
  ...
  for (let month = startMonth; month <= endMonth; month += 1) {
    ...
    const close = isTransition ? open * (1 + TRANSITION_JUMP) : randomWalkClose(open, random());

    const bodyHigh = Math.max(open, close);
    const bodyLow = Math.min(open, close);
    const bodyRange = Math.max(bodyHigh - bodyLow, open * 0.01);
    const upperWickRatio = isTransition ? 0.15 : WICK_FACTOR * random();
    const lowerWickRatio = isTransition ? 0.05 : WICK_FACTOR * random();
